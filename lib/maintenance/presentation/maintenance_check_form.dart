...
class MaintenanceCheckForm extends StatefulWidget {
  final int inspectionId;

  const MaintenanceCheckForm({Key? key, required this.inspectionId})
      : super(key: key);
...
class _MaintenanceCheckFormState extends State<MaintenanceCheckForm>
          ...
          children: [
            ...
            if (item.status != 1) ...[
              ...
              TextFormField(
                ...
                validator: (value) {
                  if (item.status != 1 && (value == null || value.isEmpty)) {
                    return 'Comentario obligatorio para este estado';
                  }
                  return null;
                },
                onFieldSubmitted: (_) {
        ...
        Row(
          children: [
            ...
            ElevatedButton(
              ...
              child: Text(
                  _editingRecommendationId != null ? 'Actualizar' : 'Agregar'),
            ),
          ],
        ),
        const SizedBox(height: 16),
...
/*         const Text(
        ...
        const SizedBox(height: 16),
        // Contenedor con altura fija y scroll para las fotos
        Container(
          height: MediaQuery.of(context).size.height *
              0.6, // 60% de la altura de la pantalla
          child: _photos.isEmpty && _isEditable
              ? const Center(child: Text('No hay fotos agregadas'))
              : ListView.builder(
                  shrinkWrap: true,
        ...
        const SizedBox(height: 16),
        // Botón para agregar fotos (siempre visible)
        if (_isEditable)
          Center(
            child: ElevatedButton.icon(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(icon: Icon(Icons.assignment), text: 'Formulario'),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              // Corrected: Changed curly braces {} to square brackets []
              children: [
                Expanded(
                  child: Text(
                    recommendation.description,
    ...
    return Scaffold(
      ...
      body: TabBarView(
        ...
        children: [
          ...
          // Segunda pestaña: Fotos y Recomendaciones
          Padding(
            padding: const EdgeInsets.all(16.0),
            child: ListView(
              children: [
      ...
      children: [
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 8),
            ElevatedButton(
              onPressed: _isEditable ? _addRecommendation : null,
              child: Text(
                  _editingRecommendationId != null ? 'Actualizar' : 'Agregar'),
            ),
          ],
        ...
        // Lista de recomendaciones con altura controlada
        Container(
          ...
          child: _recommendations.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return _buildRecommendationItem(_recommendations[index]);
                  },
                ),
        ),
      ],
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          controller: _tabController,
          tabs: const [
            Tab(icon: Icon(Icons.assignment), text: 'Checks'),
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          // Primera pestaña: Información General + Checks de Mantenimiento
          Form(
            key: _formKey,
            child: Padding(
              padding: const EdgeInsets.all(16.0),
          ...
          // Segunda pestaña: Recomendaciones
          Padding(
            padding: const EdgeInsets.all(16.0),
            child: AbsorbPointer(
              absorbing: !_isEditable,
...
class _MaintenanceCheckFormState extends State<MaintenanceCheckForm> {
  ...
  // Controladores para scroll a secciones
  ...
  Widget _buildTopNavigationBar() {
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            _buildNavButton('Información General', Icons.info, 'general'),
            _buildNavButton('Checks Mantenimiento', Icons.checklist, 'checks'),
            _buildNavButton('Fotos', Icons.photo_library, 'photos'),
            _buildNavButton(
                'Recomendaciones', Icons.recommend, 'recommendations'),
          ],
        ),
      ),
    );
  ...
  Widget _buildNavButton(String text, IconData icon, String sectionKey) {
    return Container(
  ...
  // Reemplaza el método _buildTopNavigationBar() actual con esta versión mejorada
/* 
  Widget _buildTopNavigationBar() {
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            _buildNavButton('General', Icons.info_outline, 'general'),
            _buildNavButton('Checks', Icons.checklist, 'checks'),
            _buildNavButton('Fotos', Icons.photo_library_outlined, 'photos'),
            _buildNavButton(
                'Recomendaciones', Icons.recommend_outlined, 'recommendations'),
          ],
        ),
      ),
    );
  ...
  Widget _buildNavButton(String text, IconData icon, String sectionKey) {
    return Container(
      ...
      constraints: const BoxConstraints(
        ...
        maxWidth: 140, // Ancho máximo para evitar botones muy grandes
      ),
      child: Material(
        elevation: 2,
...
// Alternativa más compacta si prefieres botones horizontales
  Widget _buildCompactTopNavigationBar() {
    return Container(
      height: 60,
      decoration: BoxDecoration(
        color: Theme.of(context).primaryColor,
      ...
      child: ListView(
        ...
        children: [
          _buildCompactNavButton(
              'Información General', Icons.info_outline, 'general'),
          _buildCompactNavButton(
              'Checks Mantenimiento', Icons.build_circle_outlined, 'checks'),
          _buildCompactNavButton(
              'Fotos', Icons.photo_camera_outlined, 'photos'),
          _buildCompactNavButton(
              'Recomendaciones', Icons.lightbulb_outline, 'recommendations'),
        ],
      ),
    );
  }

  Widget _buildCompactNavButton(String text, IconData icon, String sectionKey) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 4),
      child: Material(
        elevation: 1,
...
// Versión con indicador de sección activa (opcional)
  Widget _buildSmartNavigationBar() {
    return Container(
      height: 65,
      decoration: BoxDecoration(
        color: Theme.of(context).primaryColor,
      ...
      child: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Row(
            children: [
              _buildSmartNavButton(
                  'General', Icons.assignment_outlined, 'general', 0),
              _buildSmartNavButton('Checks', Icons.checklist_rtl, 'checks', 1),
              _buildSmartNavButton(
                  'Fotos', Icons.collections_outlined, 'photos', 2),
              _buildSmartNavButton(
                  'Recomend.', Icons.star_outline, 'recommendations', 3),
            ],
          ),
        ),
      ),
  ...
  Widget _buildSmartNavButton(
      String text, IconData icon, String sectionKey, int index) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 6),
      child: Material(
        elevation: 3,
  ...
  Widget _buildPhotosSection() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text(
          'Tipos: Antes de mantenimiento, Después de mantenimiento, Placa, Horómetro, Falla, Reparación específica',
          style: TextStyle(fontStyle: FontStyle.italic),
        ),
        const SizedBox(height: 16),
        if (_photos.isEmpty)
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  // Sección de Checks de Mantenimiento
                  _buildSectionHeader(
