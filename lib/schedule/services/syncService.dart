...
// En SyncService, modificar syncData
  Future<void> syncData() async {
    ...
    try {
      // Obtener token directamente desde SharedPreferences para mayor confiabilidad
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('auth_token');

      if (token == null) {
        print('Token no disponible para sincronización');
        _isSyncing = false;
        notifyListeners();
        return;
      }
      ...
      // Establecer el token en authService para esta instancia
      await authService.setToken(token);

      // Primero sincronizar operaciones pendientes
      await _syncPendingOperations(token);

      // Luego actualizar la lista de actividades
      await _syncActivities(token);
    } catch (e) {
  ...
  Future<void> _updateLocalActivityAfterSync(
    ...
    try {
      // Obtener la actividad local actual
      final localActivity = await dbHelper.getActivityById(activityId);
      if (localActivity == null) return;
      ...
      // Si está completamente sincronizada, obtener datos actualizados del servidor
      Activity? updatedActivity;
      if (isSynced) {
        updatedActivity = await _fetchUpdatedActivity(activityId, token);
      }

      // Actualizar actividad local
      final mergedActivity = (updatedActivity ?? localActivity).copyWith(
