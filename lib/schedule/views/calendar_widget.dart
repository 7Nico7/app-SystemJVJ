...
// Función para ordenar actividades por estado
  ...
  Widget _buildEventWidget(Activity activity, ScheduleProvider provider) {
    ...
    final statusColor = _getStatusColor(
        effectiveStatus, hourbaseInLocal, hourbaseInBackend, isExternalService);
                ...
                Row(
                  ...
                  children: [
                    Icon(
                      ...
                      _getStatusIcon(effectiveStatus, hourbaseInLocal,
                          hourbaseInBackend, isExternalService),
                      size: 10,
                      color: statusColor,
                    ),
                    SizedBox(width: 2),
  ...
  Widget _buildListActivityItem(Activity activity, ScheduleProvider provider) {
    ...
    final statusColor = _getStatusColor(
        effectiveStatus, hourbaseInLocal, hourbaseInBackend, isExternalService);
    final firstChar = _getStatusIcon(
        effectiveStatus, hourbaseInLocal, hourbaseInBackend, isExternalService);
    final hasPendingChanges = !activity.isSynced;
  ...
  IconData _getStatusIcon(
      int status, hourbaseInLocal, hourbaseInBackend, isExternalService) {
    switch (status) {
      case 1:
      ...
      case 5:
        if (isExternalService && (hourbaseInLocal || hourbaseInBackend)) {
